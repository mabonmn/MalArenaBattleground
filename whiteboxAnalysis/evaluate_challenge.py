...
class ChallengeEvaluator:
    ...
    def save_results(self, output_file="challenge_evaluation_results.csv"):
        ...
        # Create ChallengeResults directory if it doesn't exist
        results_dir = Path(__file__).parent / "ChallengeResults"
        ...
        timestamped_file = results_dir / f"{base_name}_{timestamp}{extension}"
        
        with open(timestamped_file, 'w', newline='') as csvfile:
        ...
        print(f"\nDetailed results saved to: {timestamped_file}")
        
        # Save comprehensive summary with all metrics at the top
        summary_file = results_dir / f"{base_name}_summary_{timestamp}{extension}"
        
        goodware_accuracies = []
        malware_accuracies = []
        
        # Calculate folder accuracies first
        for folder_key in sorted(self.folder_stats.keys()):
            stats = self.folder_stats[folder_key]
            accuracy = stats["correct"] / stats["total"] if stats["total"] > 0 else 0
        ...
        f1 = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0
        fpr = fp / (fp + tn) if (fp + tn) > 0 else 0
        avg_time = sum([r["time_seconds"] for r in self.results]) / len(self.results) if self.results else 0
        
        goodware_avg = sum(goodware_accuracies) / len(goodware_accuracies) if goodware_accuracies else 0
        malware_avg = sum(malware_accuracies) / len(malware_accuracies) if malware_accuracies else 0
        overall_avg = (goodware_avg + malware_avg) / 2 if goodware_accuracies and malware_accuracies else 0
        
        with open(summary_file, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            
            # METRICS AT THE TOP for easy viewing
            writer.writerow(["=== EVALUATION SUMMARY ===", "", "", ""])
            writer.writerow([])
            
            # Average Accuracies
            writer.writerow(["AVERAGE ACCURACIES", "", "", ""])
            writer.writerow(["Goodware Average", f"{goodware_avg:.1%}", f"across {len(goodware_accuracies)} folders", ""])
            writer.writerow(["Malware Average", f"{malware_avg:.1%}", f"across {len(malware_accuracies)} folders", ""])
            writer.writerow(["Overall Average", f"{overall_avg:.1%}", "", ""])
            writer.writerow([])
            
            # Overall Metrics
            writer.writerow(["OVERALL METRICS", "", "", ""])
            writer.writerow(["Total Samples", total_samples, "", ""])
            writer.writerow(["Overall Accuracy", f"{overall_accuracy:.1%}", "", ""])
            writer.writerow(["Precision", f"{precision:.1%}", "", ""])
            writer.writerow(["Recall (TPR)", f"{recall:.1%}", "", ""])
            writer.writerow(["F1-Score", f"{f1:.1%}", "", ""])
            writer.writerow(["False Pos Rate", f"{fpr:.1%}", "", ""])
            writer.writerow(["Avg Time/Sample", f"{avg_time:.2f}s", "", ""])
            writer.writerow([])
            
            # Confusion Matrix
            writer.writerow(["CONFUSION MATRIX", "", "", ""])
            writer.writerow(["True Positives", tp, "Malware correctly identified", ""])
            writer.writerow(["False Positives", fp, "Goodware incorrectly flagged", ""])
            writer.writerow(["True Negatives", tn, "Goodware correctly identified", ""])
            writer.writerow(["False Negatives", fn, "Malware missed", ""])
            writer.writerow([])
            writer.writerow(["=" * 50, "", "", ""])
            writer.writerow([])
            
            # Per-folder breakdown at the bottom
            writer.writerow(["PER-FOLDER BREAKDOWN", "", "", ""])
...
def main():
    ...
    parser.add_argument("--output", default="challenge_evaluation_results.csv",
                       help="Base name for output CSV files (timestamp will be added automatically)")
