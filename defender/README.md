...
## MalConv Torch Model (optional)

This repository now includes a minimal PyTorch MalConv-like model (`defender/models/malconv_model.py`). If a weights file exists at `defender/models/malconv_model.pt`, the web service will load and serve that model; otherwise it falls back to the original NFS model.

Environment variables:

- `DF_MALCONV_WEIGHTS` (default `models/malconv_model.pt`)
- `DF_MALCONV_THRESH` (default `0.5`)
- `DF_MALCONV_MAX_BYTES` (default `1048576`)

Note: The Docker image installs a CPU-only PyTorch to remain within resource limits. The service reads up to `DF_MALCONV_MAX_BYTES` from a POSTed PE file, runs the model, and returns `{ "result": 0|1 }`.
