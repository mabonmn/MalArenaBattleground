"""
Orchestrator Version 1 - Mutates after every step

This version uses metame to mutate PE files after each processing step.
"""
...
def setup_logging():
    ...
    log_file = f"orchestrator_v1_{timestamp}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def main():
    """Main orchestrator function - Version 1: Mutate after every step."""
    parser = argparse.ArgumentParser(
        description='PE Modification Orchestrator v1 - Mutates after every step'
    )
    ...
    parser.add_argument('--output-dir', default='output_v1', help='Base output directory')
    ...
    logging.info("=== PE MODIFICATION ORCHESTRATOR V1 ===")
    logging.info("Strategy: Mutate after every processing step")
    ...
    try:
        ...
        # Create working directories for each step
        working_dir = output_base / f"working_{timestamp}"
        after_strings_dir = output_base / f"after_strings_{timestamp}"
        mutated_strings_dir = output_base / f"mutated_strings_{timestamp}"
        after_caves_dir = output_base / f"after_caves_{timestamp}"
        mutated_caves_dir = output_base / f"mutated_caves_{timestamp}"
        after_packing_dir = output_base / f"after_packing_{timestamp}"
        mutated_packing_dir = output_base / f"mutated_packing_{timestamp}"
        final_dir = output_base / f"final_{timestamp}"

        for directory in [output_base, working_dir, after_strings_dir, mutated_strings_dir,
                         after_caves_dir, mutated_caves_dir, after_packing_dir, 
                         mutated_packing_dir, final_dir]:
        ...
        # Phase 1: String Injection + Mutation
        if not args.skip_strings:
            run_string_injection(current_dir, after_strings_dir, args.source_dir)
            run_mutation(after_strings_dir, mutated_strings_dir, args.mutation_level)
            current_dir = mutated_strings_dir
        else:
            logging.info("String injection skipped")
            for file_path in current_dir.glob("*"):
                if file_path.is_file():
                    shutil.copy2(file_path, mutated_strings_dir / file_path.name)
            current_dir = mutated_strings_dir

        # Phase 2: Code Cave Insertion + Mutation
        if not args.skip_caves:
            run_code_cave_insertion(current_dir, after_caves_dir, args.source_dir)
            run_mutation(after_caves_dir, mutated_caves_dir, args.mutation_level)
            current_dir = mutated_caves_dir
        else:
            logging.info("Code cave insertion skipped")
            for file_path in current_dir.glob("*"):
                if file_path.is_file():
                    shutil.copy2(file_path, mutated_caves_dir / file_path.name)
            current_dir = mutated_caves_dir

        # Phase 3: Packing + Mutation
        if not args.skip_packing:
            run_packing(current_dir, after_packing_dir, args.packer)
            run_mutation(after_packing_dir, mutated_packing_dir, args.mutation_level)
            current_dir = mutated_packing_dir
        else:
            logging.info("Packing skipped")
            for file_path in current_dir.glob("*"):
                if file_path.is_file():
                    shutil.copy2(file_path, mutated_packing_dir / file_path.name)
            current_dir = mutated_packing_dir
        ...
        logging.info("=== ORCHESTRATION V1 COMPLETED ===")
