"""
Code Cave Inserter with Activity Logging
...
def inject_code_safely(target_file, cave, code_chunk, output_file):
    """Inject code with comprehensive error handling - GUARANTEED output creation."""
    logger = logging.getLogger()
    ...
    if len(code_chunk['data']) > cave['size']:
        logger.error(f"Code chunk too large for cave: {len(code_chunk['data'])} > {cave['size']}")
        ensure_output_exists()
        return False, "Code chunk too large for cave"
    ...
    try:
        with open(output_file, 'r+b') as f:
            ...
            # Write code chunk
            logger.debug(f"Writing {len(code_chunk['data'])} bytes of code")
            f.write(code_chunk['data'])

            # Fill remaining space with zeros
            remaining = cave['size'] - len(code_chunk['data'])
            if remaining > 0:
        ...
        if Path(output_file).exists() and Path(output_file).stat().st_size > 0:
            result_msg = f"Injected {len(code_chunk['data'])} bytes into {cave['section']}"
...
def main():
    ...
    logger = logging.getLogger()
    logging.info("Code cave_inserter")

    print("Code Cave Inserter with Activity Logging")
    ...
    try:
        ...
        for i, (cave, chunk) in enumerate(zip(caves, code_chunks)):
            ...
            success, msg = inject_code_safely(current_file, cave, chunk, temp_output)
